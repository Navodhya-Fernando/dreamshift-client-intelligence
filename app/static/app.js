...
function queryPlanMarkup(payload) {
  ...
  return `
    <details class="query-plan-details">
      <summary>View Airtable query logic</summary>
      <div class="query-plan-grid">
        <span class="query-pill accent">${escapeHtml(humanizePlanKey(plan.intent || 'summary'))}</span>
...
function renderQueryChart(containerId, chartSpec) {
  if (!chartSpec || chartSpec.type === 'none') return;
  const theme = chartTheme();
  if (chartSpec.type === 'bar') {
    const horizontal = chartSpec.orientation !== 'vertical';
    renderChart(containerId, {
      ...theme,
      chart: { ...theme.chart, type: 'bar', height: Math.max(260, (chartSpec.categories?.length || 1) * (horizontal ? 38 : 22)) },
      series: chartSpec.series || [],
      xaxis: { ...theme.xaxis, categories: chartSpec.categories || [] },
      plotOptions: { bar: { horizontal, borderRadius: 9, barHeight: horizontal ? '62%' : undefined, columnWidth: horizontal ? undefined : '58%' } },
      dataLabels: { enabled: true, style: { colors: ['#ffffff'], fontWeight: 700 }, formatter: value => prettyInt(value) },
      legend: { show: false },
    });
    return;
  }
  if (chartSpec.type === 'donut') {
    renderChart(containerId, {
      ...theme,
      chart: { ...theme.chart, type: 'donut', height: 300 },
      series: chartSpec.series || [],
      labels: chartSpec.labels || [],
      legend: { position: 'bottom' },
      plotOptions: { pie: { donut: { size: '68%', labels: { show: true, total: { show: true, label: 'Filtered clients' } } } } },
    });
    return;
  }
  if (chartSpec.type === 'heatmap') {
    renderChart(containerId, {
      ...theme,
      chart: { ...theme.chart, type: 'heatmap', height: Math.max(300, (chartSpec.series?.length || 1) * 46) },
      series: chartSpec.series || [],
      ...
      plotOptions: {
        heatmap: {
          ...
          colorScale: { ranges: [
            ...
            { from: 1, to: 2, color: '#593270', name: '1–2' },
            { from: 3, to: 5, color: '#8c4bff', name: '3–5' },
            { from: 6, to: 9999, color: '#f6b900', name: '6+' },
          ] },
        },
      },
    });
  ...
  if (chartSpec.type === 'grouped_bar') {
    renderChart(containerId, {
      ...theme,
      chart: { ...theme.chart, type: 'bar', height: Math.max(300, (chartSpec.categories?.length || 1) * 42) },
      series: chartSpec.series || [],
      xaxis: { ...theme.xaxis, categories: chartSpec.categories || [] },
      plotOptions: { bar: { horizontal: true, borderRadius: 7, barHeight: '70%' } },
      legend: { position: 'bottom' },
    });
  }
}
...
async function askQuestion(question) {
  ...
  const loading = addMessage('assistant', `
    <div class="message-label">DreamShift Intelligence</div>
    <h4>Building a validated Airtable query…</h4>
    <p>I’m identifying the exact field, applying your dashboard filters, calculating the result in Python and preparing the most useful visual.</p>
  `);
  ...
  try {
    ...
    const payload = await response.json();
    if (!response.ok) throw new Error(payload.detail || `Query API failed: ${response.status}`);

    const chartId = `queryChart_${++APP.chatCounter}`;
    const chartAvailable = payload.chart && payload.chart.type !== 'none';
    const result = payload.result || {};
    const coverage = result.coverage_percentage;
    const coverageMarkup = coverage == null ? '' : `
      <div class="query-coverage">
        <strong>${escapeHtml(String(coverage))}% field coverage</strong>
        <span>${prettyInt(result.populated_clients || 0)} of ${prettyInt(result.denominator || payload.filtered_client_count || 0)} filtered clients have populated data for this field.</span>
      </div>`;

    loading.innerHTML = `
      <div class="message-label">Airtable query ${payload.model ? `• ${escapeHtml(payload.model)}` : '• deterministic engine'}</div>
      <h4>${escapeHtml(payload.chart?.title || humanizePlanKey(payload.query_plan?.dimension || 'Client intelligence'))}</h4>
      <p class="query-answer">${escapeHtml(payload.answer || 'No answer returned.')}</p>
      ${queryPlanMarkup(payload)}
      ${coverageMarkup}
      ${chartAvailable ? `<div class="ai-chart-card query-chart-card"><span class="eyebrow">Live query result</span><div id="${chartId}" class="ai-chart query-chart"></div></div>` : ''}
      ${payload.key_findings?.length ? `
        <div class="ai-metric query-findings">
          <span class="eyebrow">Key findings</span>
          <ul>${payload.key_findings.map(item => `<li>${escapeHtml(item)}</li>`).join('')}</ul>
        </div>` : ''}
      <div class="ai-grid">
        ...
        <div class="ai-metric"><span class="eyebrow">Business implication</span><p>${escapeHtml(payload.marketing_opportunity || 'No business implication returned.')}</p></div>
      </div>
      <div class="query-data-note">${escapeHtml(payload.data_note || '')}</div>
    `;
    if (chartAvailable) renderQueryChart(chartId, payload.chart);
  } catch (error) {
    loading.innerHTML = `<div class="message-label">DreamShift Intelligence</div><h4>Couldn’t complete the Airtable query</h4><p>${escapeHtml(error.message || String(error))}</p>`;
  }
}
...
async function bootstrap() {
  bindEvents();
  setView('overview');
  addMessage('assistant', `<div class="message-label">Welcome</div><h4>Ask DreamShift’s portfolio a question</h4><p>Ask a question in plain English. I will identify the exact Airtable fields, execute a validated query and return the result with an appropriate chart.</p>`);
